...
# =========================================================
# 4. IMMORTAL NEOX SSH TERMINAL BRIDGE (INTERACTIVE PATCH)
# =========================================================
...
async def websocket_terminal(websocket: WebSocket, user_id: str = "user_001"):
    ...
    try:
        ...
        async with asyncssh.connect(ssh_host, port=int(ssh_port), username="root", client_keys=[master_key], known_hosts=None, login_timeout=15) as conn:
            ...
            # 🚨 THE FIX: Force /bin/bash to start interactively and merge output streams 🚨
            async with conn.create_process('/bin/bash', term_type='xterm-256color', term_size=(50, 20), encoding='utf-8', stderr=asyncssh.STDOUT) as process:
                await websocket.send_text("\r\n\033[32m[SUCCESS] Datacenter Door Unlocked. Bash Shell Initialized.\033[0m\r\n")
                ...
                async def read_from_ssh():
                    try:
                        ...
                        await websocket.send_text(f"\r\n\033[31m[SSH Read Warning] {str(e)}\033[0m\r\n")
                ...
                async def write_to_ssh():
                    try:
                        while True:
                            ...
                            # 🚨 THE FIX: Simulate pressing the Enter key (\r) if the UI forgot it 🚨
                            if not user_input.endswith('\r') and not user_input.endswith('\n'):
                                user_input += '\r'
