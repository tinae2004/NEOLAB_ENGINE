...
load_dotenv()
app = FastAPI(title="NEO LAB Engine", version="7.2.0 (Master Tunnel Edition)")
...
# =========================================================
# 4. NEOX GHOST TUNNEL (NOW SECURED WITH PRIVATE KEY)
# =========================================================
...
async def websocket_terminal(websocket: WebSocket, user_id: str = "user_001"):
    ...
    if mode == "beginner":
        ...
        try:
            # 🚨 THE MASTER UPGRADE: Loading the Key from the Render Vault
            ...
            # Forging the master key into the SSH connection
            ...
            await websocket.send_text(f"\r\n\033[31m[ERROR] Datacenter Connection Interrupted: {str(e)}\033[0m\r\n")
