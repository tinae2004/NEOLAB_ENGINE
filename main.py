...
load_dotenv()
app = FastAPI(title="NEO LAB Engine", version="12.0 (Xterm.js Master Route)")
...
# =========================================================
# UI ROUTE - Serving your HTML perfectly
# =========================================================
...
# =========================================================
# 4. IMMORTAL NEOX SSH TERMINAL BRIDGE (XTERM.JS RAW ROUTE)
# =========================================================
...
async def websocket_terminal(websocket: WebSocket, user_id: str = "user_001"):
    ...
    if mode == "beginner":
        ...
        try:
            # 🚨 PURE XTERM.JS ROUTE: Opening /bin/bash -l for interactive raw pipe 🚨
            async with asyncssh.connect(ssh_host, port=int(ssh_port), username="root", client_keys=[master_key], known_hosts=None, login_timeout=15) as conn:
                async with conn.create_process('/bin/bash -l', term_type='xterm-256color', term_size=(80, 24), encoding='utf-8') as process:
                    ...
                    async def write_to_ssh():
                        try:
                            while True:
                                # Raw pass-through, no artificial formatting
