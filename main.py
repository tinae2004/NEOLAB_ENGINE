...
load_dotenv()
app = FastAPI(title="NEO LAB Engine", version="7.3.0 (Master Tunnel Edition)")
...
async def scan_market(vram: float, storage: float = 100.0, gpu_name: str = None):
    try:
        ...
        # 🚨 BANNED HOSTS: Block broken Datacenters
        BANNED_HOSTS = [385807] 
        ...
        try:
            ...
            # 🚨 EXPANDED LIST: Show top 25 machines instead of just 5
...
# =========================================================
# 4. NEOX GHOST TUNNEL (PATIENT KNOCK EDITION)
# =========================================================
                ...
                try:
                    async with asyncssh.connect(ssh_host, port=int(ssh_port), username="root", client_keys=[master_key], known_hosts=None) as conn:
                        async with conn.create_process(term_type='xterm-256color', term_size=(50, 20)) as process:
                            ...
                            break  # Exit the retry loop if connection stays open
                            
                except (OSError, asyncssh.Error) as e:
                    if attempt < max_retries - 1:
                        # Wait 5 seconds and knock again
                        await websocket.send_text(f"\r\n\033[33m[SYS] Engine booting (Attempt {attempt+1}/{max_retries}). Retrying in 5s...\033[0m\r")
                        await asyncio.sleep(5)
                    else:
                        await websocket.send_text(f"\r\n\033[31m[ERROR] Datacenter refused connection after 40 seconds: {str(e)}\033[0m\r\n")
